...
import { Key, Bell, Zap, Copy, Check, Info, Save, Eye, EyeOff } from "lucide-react";
...
const PROVIDERS = [
  ...
  },
  {
    name: "Google Gemini",
    env: "GOOGLE_API_KEY",
    models: ["gemini-2.0-flash", "gemini-1.5-pro", "gemini-1.5-flash", "gemini-2.0-flash-thinking"],
    color: "#4285f4",
...
const PROXY_ENDPOINTS = [
  { method: "POST", path: "/proxy/openai/v1/chat/completions",   provider: "OpenAI",    note: "Drop-in replacement" },
  { method: "POST", path: "/proxy/anthropic/v1/messages",         provider: "Anthropic", note: "Anthropic SDK format" },
...
function SmtpForm({ onSaved }: { onSaved: () => void }) {
  const [form, setForm] = useState({ smtp_host: "", smtp_port: "587", smtp_user: "", smtp_password: "", alert_from_email: "" });
  ...
  useEffect(() => {
    api.settings.get().then((s: SiteSettings) => {
      setForm({
  ...
  return (
    <div className="px-5 py-4 flex flex-col gap-4">
      <p className="text-xs text-[--muted-fg]">
        Configure your SMTP server to receive email alerts when a budget threshold is reached.
      </p>

      <div className="grid grid-cols-2 gap-3">
        {/* SMTP_HOST */}
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[--amber] uppercase tracking-wider">SMTP_HOST</label>
          <input
            style={inputStyle}
        ...
        {/* SMTP_PORT */}
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[--amber] uppercase tracking-wider">SMTP_PORT</label>
          <input
            style={inputStyle}
            placeholder="587"
            type="number"
        ...
        {/* SMTP_USER */}
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[--amber] uppercase tracking-wider">SMTP_USER</label>
          <input
            style={inputStyle}
        ...
        <div className="flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[--amber] uppercase tracking-wider">
            SMTP_PASSWORD
            {passwordSet && <span className="ml-2 text-green-400 normal-case">● configured</span>}
          </label>
          <div className="relative">
            <input
              style={{ ...inputStyle, paddingRight: 32 }}
              type={showPass ? "text" : "password"}
              placeholder={passwordSet ? "••••••••  (leave blank to keep)" : "App password"}
            ...
            <button
              ...
              style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", color: "var(--muted-fg)", background: "none", border: "none", cursor: "pointer", padding: 0 }}
            >
        ...
        <div className="col-span-2 flex flex-col gap-1">
          <label className="font-mono text-[10px] text-[--amber] uppercase tracking-wider">ALERT_FROM_EMAIL</label>
          <input
            style={inputStyle}
            placeholder="alerts@yourapp.io"
            value={form.alert_from_email}
            onChange={(e) => setForm((f) => ({ ...f, alert_from_email: e.target.value }))}
          />
        </div>
      </div>
        ...
        <div className="flex flex-col gap-5">
          ...
          >
            ...
            <p className="text-xs text-[--amber]/80 leading-relaxed">
              API keys are stored on the backend only. They are never sent to the browser.
              Only the project{" "}
              <code className="font-mono bg-[--amber]/10 px-1 rounded">bf-xxx</code>{" "}
          ...
          >
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[--border]">
              ...
              <h2 className="font-heading font-700 text-sm">Provider API Keys</h2>
                ...
                <div key={p.name} className="px-5 py-4 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[10px] text-[--muted-fg] italic">{p.note}</span>
                    </div>
                    {/* Env var */}
          ...
          >
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[--border]">
              ...
              <h2 className="font-heading font-700 text-sm">Alert Configuration</h2>
              <span className="ml-auto text-[10px] text-[--muted-fg] bg-white/5 px-2 py-0.5 rounded-full">
                Optional
              </span>
            </div>
            <SmtpForm onSaved={() => setToast({ show: true, message: "Settings saved" })} />
          </motion.div>
