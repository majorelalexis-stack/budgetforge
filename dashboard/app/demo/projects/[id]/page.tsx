...
const MODELS_BY_PROVIDER: Record<string, string[]> = {
  ...
  mistral:        ["mistral-large-latest", "mistral-small-latest", "mistral-nemo", "codestral-latest"],
  openrouter:     ["openrouter/anthropic/claude-3.5-sonnet", "openrouter/openai/gpt-4", "openrouter/google/gemini-pro"],
  together:       ["togethercomputer/llama-3-70b", "togethercomputer/mistral-7b", "togethercomputer/falcon-40b"],
  "azure-openai": ["azure/gpt-4o", "azure/gpt-4o-mini", "azure/gpt-4-turbo"],
  "aws-bedrock":  ["anthropic.claude-3-sonnet", "anthropic.claude-3-opus", "meta.llama3-70b-instruct"],
...
const DEMO_DAILY = Array.from({ length: 30 }, (_, i) => {
  ...
  const spend =
    i < 5
      ? 0
      : Math.max(0, 0.8 + Math.sin(i * 0.7) * 0.5 + Math.random() * 0.4);
...
const DEMO_PROJECTS: Record<
  string,
  { name: string; used: number; budget: number; chain: string[] }
> = {
  "ai-chat-assistant": {
  ...
  "web-scraper-agent": {
    ...
    used: 22.1,
    budget: 75,
    chain: ["openai/gpt-4o-mini", "openai/gpt-3.5-turbo"],
  },
  "image-analysis": {
    name: "Image Analysis",
    used: 156.33,
...
export default function DemoProjectPage() {
  ...
  const projectData =
    DEMO_PROJECTS[projectId] || DEMO_PROJECTS["ai-chat-assistant"];
          ...
          >
            {[
              ...
              },
              {
                ...
                label: "Action",
                value: action,
                color: action === "block" ? "#ef4444" : "#3b82f6",
              },
              {
                icon: Key,
                label: "Alert at",
              ...
              >
                ...
                <span
                  className="font-mono text-xs font-500 capitalize"
                  style={{ color }}
                >
                  {value}
                </span>
                  ...
                  <div key={e.name}>
                    ...
                    <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${pct.toFixed(1)}%`, background: e.color, opacity: 0.75 }}
                      />
                    </div>
                  </div>
                );
